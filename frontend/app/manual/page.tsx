...
const GENERAL_ITEMS: ManualItem[] = [
  {
    ...
    points: [
      ...
      '"오늘의 기록" 영역에서 오늘 출퇴근·재출근/재퇴근·외출/복귀 시간을 모두 확인할 수 있어요.',
      "이번 주 누적 근무시간이 표시되고, 주 52시간을 넘기면 경고가 떠요.",
      "매주 월요일에는 지난주 근무 추이를 AI가 분석해서, 회사가 정한 주·월 최대 근로시간을 초과할 것 같으면 알림으로 미리 알려줘요(주 4일 이상 출근한 경우에만 분석해요).",
  ...
  },
  {
    ...
    title: "재출근 · 재퇴근",
    points: [
      "정상 퇴근을 한 뒤에도 다시 근무해야 하면, 메인 카드의 퇴근 버튼 위 \"재출근\" 뱃지를 눌러 재출근할 수 있어요. 재출근은 하루에 한 번만 가능해요.",
      "재출근 중에는 버튼이 \"🔁 재출근중\"으로 바뀌고, 다시 퇴근할 때는 \"🔁 재퇴근\" 버튼을 눌러요.",
      "재퇴근하면 팀장/관리자에게 승인 요청이 가고, 승인이 완료돼야 그 시간이 근무시간에 반영돼요. 반려되면 사유를 확인할 수 있어요.",
      "메인 카드와 \"오늘의 기록\"에서 재출근/재퇴근 시각과 승인 상태(승인대기중 · 승인완료 · 반려됨)를 확인할 수 있어요.",
    ],
  },
  {
    icon: "🚶",
    title: "외출 · 복귀",
    points: [
      "출근 후 퇴근 전에만 메인 카드 상단에 \"🚶 외출하기\" 버튼이 보여요. 잠깐 자리를 비울 때 눌러서 기록해요.",
      "외출 중에는 버튼이 \"🏃 복귀하기\"로 바뀌고, 돌아오면 눌러서 복귀를 기록해요.",
      "하루 동안의 외출 시간 합계는 실제 근무시간 계산에서 자동으로 제외돼요.",
      "\"오늘의 기록\"에서 외출/복귀 시각과 외출 시간(분)을 확인할 수 있어요.",
    ],
  },
  {
    icon: "🎙️",
    title: "AI 회의록 (베타)",
    points: [
      '메인 메뉴의 "AI 회의록"에서 "새 회의 녹음하기"를 누르면 바로 녹음이 시작돼요. 최대 30분까지 녹음할 수 있어요.',
      '녹음 대신 "직접 타이핑해서 작성하기"를 누르면 회의 내용을 글로 적어 등록할 수 있어요. 마찬가지로 AI가 요약과 할 일 목록을 정리해줘요.',
      "녹음 파일은 이 기기에만 저장되고 서버에는 보관되지 않아요. 종료 버튼을 누르면 그 순간 서버로 전송돼 텍스트 추출과 AI 요약, 할 일 목록 정리가 자동으로 진행돼요.",
      "처리 중에는 화면을 나가거나 앱을 꺼도 괜찮아요. 완료되면 알림으로 알려드리고, 회의록 목록에서 바로 확인할 수 있어요.",
      "요약 내용과 할 일 목록은 자유롭게 수정할 수 있어요. AI가 처음 만든 내용과 달라진 부분은 색으로 표시돼요 (노란색 강조 = 추가된 내용, 취소선 = 삭제된 내용).",
      "소속 회사가 있으면 같은 회사 동료들과 회의록이 함께 공유되고, 소속 회사가 없으면 나만 보는 개인 모드로 사용돼요.",
      "베타 기간에는 계정당 월 3회까지 무료로 이용할 수 있어요. 다 사용하면 다음 달까지 기다리시거나 안내 메시지를 확인해주세요.",
    ],
  },
  {
    icon: "🏖️",
...
const MANAGER_ITEMS: ManualItem[] = [
  ...
  },
  {
    ...
    title: "재출근 승인",
    points: [
      '"재출근" 탭에서 팀원의 재퇴근 신청을 확인하고 "승인"/"반려"할 수 있어요. 반려 시 사유를 남길 수 있어요.',
      "진행 중인 재출근, 승인 대기중, 승인/반려 완료 건이 구분되어 표시돼요.",
    ],
  },
  {
    icon: "📋",
...
const ADMIN_ITEMS: ManualItem[] = [
  ...
  },
  {
    ...
    title: "근로시간 패턴 알림 기준",
    points: [
      '"근로시간 패턴 알림 기준"에서 회사의 주 최대/월 최대 근로시간(시간 단위)을 설정할 수 있어요. 주 최대는 기본 52시간, 월 최대는 비워두면 검사하지 않아요.',
      "매주 월요일 오전, 지난주에 4일 이상 근무한 직원만 추려 AI가 최근 4주 근무 추이를 분석해요. 설정한 기준을 넘길 것 같으면 해당 직원과 소속 팀장·관리자에게 알림이 자동으로 가요.",
    ],
  },
  {
    icon: "👥",
