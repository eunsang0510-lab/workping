...
def validate_checkin_location(req: CheckInValidateRequest, db: Session = Depends(get_db)):
    ...
    nearest = min(locations, key=lambda l: calc_distance(req.latitude, req.longitude, l.latitude, l.longitude))
    nearest_distance = int(calc_distance(req.latitude, req.longitude, nearest.latitude, nearest.longitude))
